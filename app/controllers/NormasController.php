...
class NormasController extends JoshController
{
    ...
    {
        $input = Input::all();
        $file = Input::file('ruta_norma');
        $validation = Validator::make($input, Norma::$rules);

        if ($validation->passes() && !is_null($file)) {
            $path = storage_path('normas' . DIRECTORY_SEPARATOR . Input::get('id_local'));
            ...
            $input['ruta_norma'] = $path . DIRECTORY_SEPARATOR . $file->getClientOriginalName();
            $input['fecha_hora'] = \Carbon\Carbon::now();
            $input['nombre_usuario'] = Auth::user()->nombre;
